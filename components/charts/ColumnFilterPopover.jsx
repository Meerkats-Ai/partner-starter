    ...
    return (
        <div className="flex items-center gap-1.5">
            <select
                ...
                className="h-8 w-28 shrink-0 truncate rounded-lg border border-border bg-card px-2 text-xs text-foreground focus:border-border focus:outline-none"
            >
                {columns.map((c) => (
            ...
            <select
                ...
                className="h-8 w-24 shrink-0 rounded-lg border border-border bg-card px-1.5 text-xs text-foreground focus:border-border focus:outline-none"
            >
            ...
            <input
                ...
                className="h-8 min-w-0 flex-1 rounded-lg border border-border px-2 text-xs focus:border-border focus:outline-none"
            />
            <button
                type="button"
                onClick={onRemove}
                title="Remove filter"
                className="flex h-8 w-7 shrink-0 items-center justify-center rounded-lg text-muted-foreground hover:bg-muted hover:text-foreground"
            >
    ...
    return (
        <div className="relative" ref={ref}>
            ...
            >
                ...
                {active && (
                    <span className="grid h-4 min-w-[16px] place-items-center rounded-full bg-primary px-1 text-[9px] font-bold text-primary-foreground">
                        {activeCount}
                    </span>
                )}
            </button>
            ...
            {open && (
                <div className="absolute right-0 z-40 mt-1 w-[26rem] rounded-xl border border-border bg-popover p-3 shadow-lg">
                    ...
                    {(draft || []).length === 0 ? (
                        <p className="px-1 py-2 text-xs text-muted-foreground">No column filters yet.</p>
                    ) : (
                    ...
                    <button
                        type="button"
                        onClick={addRule}
                        className="mt-2 flex w-full items-center justify-center gap-1 rounded-lg border border-dashed border-border py-1.5 text-xs font-medium text-muted-foreground transition hover:border-primary/30 hover:text-primary"
                    >
                    ...
                    <div className="mt-3 flex items-center justify-end gap-2 border-t border-border pt-3">
                        ...
                        <button type="button" onClick={apply}
                            className="rounded-lg bg-primary px-4 py-1.5 text-xs font-semibold text-primary-foreground transition hover:bg-primary/90">
                            Apply
                        </button>
                    </div>
                </div>
